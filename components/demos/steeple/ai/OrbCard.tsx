...
import type { OrbDef } from '@/data/steeple/ai-platform';
...
export function OrbCard({ orb }: { orb: OrbDef }) {
  const Icon = iconMap[orb.icon] ?? Activity;
  const dotColor = statusColors[orb.status];

  return (
    <Link
      href={orb.href}
      className="group relative flex flex-col overflow-hidden rounded-xl border border-gray-100 bg-white shadow-[0_1px_3px_rgba(82,35,152,0.06)] transition-all duration-200 hover:shadow-[0_4px_16px_rgba(82,35,152,0.12)]"
    >
      {/* Gradient top border */}
      <div
        className="h-1"
