...
export function DecisionsView() {
  ...
  return (
    <>
      <div style={{ marginBottom: 12, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12 }}>
        {summaryTiles.map((t) => (
          <div key={t.label} className="pg-card-elevated" style={{ padding: '14px 18px' }}>
          ...
          {DECISIONS.map((d, i) => {
            ...
            return (
              ...
              >
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, marginBottom: 5, flexWrap: 'wrap' }}>
                    <div style={{ flex: 1, minWidth: 260 }}>
                      ...
                      <h3 style={{ fontSize: 17, fontWeight: 700, color: '#ffffff', marginBottom: 5, lineHeight: 1.3 }}>{d.title}</h3>
                    </div>
                    <span style={{ height: 'fit-content' }}><StatusBadge status={d.decision} /></span>
                  </div>
                  <p style={{ fontSize: 14, color: '#ffffff', lineHeight: 1.55, marginBottom: 8 }}>{d.rationale}</p>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14, color: '#ffffff', flexWrap: 'wrap', gap: 12, paddingTop: 8, borderTop: '1px solid rgba(255,255,255,0.16)' }}>
