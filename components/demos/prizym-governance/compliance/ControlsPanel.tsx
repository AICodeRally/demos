...
import { COMPLIANCE_CONTROLS, type ComplianceControl } from '@/data/prizym-governance/oversee';
import { ShieldCheck, AlertTriangle, XCircle, Clock, FileCheck, CheckCircle2 } from 'lucide-react';
...
const WEIGHTS: Record<ComplianceControl['status'], number> = {
  compliant: 1.0, at_risk: 0.5, non_compliant: 0, not_tested: 0.3,
};

export function ControlsPanel({ showGauge = true }: { showGauge?: boolean } = {}) {
  ...
  const categories = useMemo(() => Array.from(new Set(controls.map(c => c.category))), [controls]);
  ...
  function markTested(id: string) {
    ...
    setControls(prev => prev.map(c => c.id === id ? { ...c, status: 'compliant', lastTested: today, evidence: c.evidence + 1 } : c));
    const item = controls.find(c => c.id === id);
    showDemoToast(`${item?.code ?? ''} tested · marked compliant`, 'success');
  }
    ...
    <>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 24, flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: 1, minWidth: 280 }}>
              ...
              <p style={{ fontSize: 15, color: '#ffffff', lineHeight: 1.5, marginBottom: 14 }}>
                Weighted score across {controls.length} controls in {categories.length} categories — SOX / ICFR, wage and hour, tax, and data security.
              </p>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 14, flexWrap: 'wrap' }}>
        {(['all', ...categories] as string[]).map((cat) => {
          ...
          const count = cat === 'all' ? controls.length : controls.filter(c => c.category === cat).length;
          return (
            <button
              key={cat}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead style={{ position: 'sticky', top: 0, background: 'rgba(15, 23, 42, 0.72)', backdropFilter: 'blur(12px)', zIndex: 1 }}>
              <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.24)' }}>
                ...
                <th style={{ padding: '14px 16px', textAlign: 'right', fontSize: 14, fontWeight: 800, color: '#f1f5f9', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Action</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((c, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td style={{ padding: '14px 16px', textAlign: 'right' }}>
                      ...
                      >
                        <CheckCircle2 size={13} strokeWidth={2.8} /> Mark Tested
                      </button>
                    </td>
                  </tr>
                );
