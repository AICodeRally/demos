...
import type { CoachingCardData } from '@/data/equipr/ai-platform';
...
export function CoachingCard({ card }: { card: CoachingCardData }) {
  ...
  const Icon = iconMap[card.icon] ?? Truck;
  const pStyle = priorityStyles[card.priority] ?? priorityStyles.MEDIUM;

  return (
    <div className="rounded-xl border border-gray-100 bg-white shadow-[0_1px_3px_rgba(37,99,235,0.06)] transition-all duration-200">
      <div className="p-5">
        {/* Header row */}
        ...
        {stepsExpanded && (
          <ol className="mt-2 space-y-1.5 pl-4">
            {card.actionSteps.map((step, i) => (
              <li
                key={i}
                className="list-decimal text-xs text-gray-700 marker:font-semibold marker:text-blue-400"
              >
                {step}
              </li>
            ))}
        ...
        <div className="mt-4 flex items-center gap-2">
          <button className="rounded-lg bg-[#2563EB] px-3 py-1.5 text-[11px] font-semibold text-white transition-colors hover:bg-[#1d4ed8]">
            Take Action
          </button>
