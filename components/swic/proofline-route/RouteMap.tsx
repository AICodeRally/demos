...
import { ROUTE_STOPS, STOP_TYPE_COLORS } from '@/lib/swic/data/proofline-route-data';
...
export function RouteMap({ activeStopIndex, onStopClick }: RouteMapProps) {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      ROUTE_STOPS.forEach((stop, i) => {
        ...
        // Account name tooltip for active
        if (isActive) {
  ...
  // Update markers when active stop changes
  useEffect(() => {
