...
import { STRATEGY_TAG_CONFIG } from '@/lib/swic/data/proofline-route-data';
import type { CalculationResult } from '@/lib/swic/engine/types';

/* ══════════════════════════════════════════════════════════
   Commission Panel — "Strategy Meets Street"

   Wider vertical right pane showing:
   1. Day Total with animated commission + delta
   2. 2026 Strategy scorecard (margin %, mix, pipeline)
   3. AI suggestion cards with integrated strategy + coaching
   4. Accept All button

   All font sizes use em units — scales with parent fontSize.
   ══════════════════════════════════════════════════════════ */
...
export function CommissionPanel({
  ...
  // Count strategy metrics across all accepted suggestions (all stops)
  ...
  // Collapsed: show just a toggle tab
  if (collapsed) {
    return (
      <button
        onClick={onToggle}
      ...
      <div className="px-3 py-2 flex-shrink-0" style={{ borderBottom: '1px solid var(--pl-zone-border)' }}>
        ...
        <div className="grid grid-cols-2 gap-1.5">
          <StrategyMiniCard
            ...
            value="6% target"
            color="#C6A052"
            active={allAcceptedStrategies.length > 0}
          />
          <StrategyMiniCard
            label="Mix Shift"
            value="Craft +15%"
...
/* ── Strategy Suggestion Card (integrated) ─────────────── */
