...
- Demo apps under `app/(demos)`:
  ...
  - 1 (`proofline-route`) is intentionally standalone and does not use shell config
- Shell systems currently in use:
  - `DemoShell`: most demos
  - `SpmShell`: `quota`
  - standalone custom layout: `proofline-route`
...
- Existing demos show richer config and varied patterns:
  - `app/(demos)/proofline/demo.config.ts`
...
- `app/(demos)/proofline-route/layout.tsx` (custom, no shell)
