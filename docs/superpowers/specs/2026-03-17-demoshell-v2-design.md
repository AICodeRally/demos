...
| proofline | DemoShell | Yes | Migrate to sidebar |
| proofline-andrews | DemoShell | Yes | Migrate to sidebar (fork of proofline with different nav hrefs) |
| proofline-route | None (standalone) | No | Skip — standalone page, no shell wrapper. Add `meta` export for registry. |
...
`proofline-route` is a standalone single-page demo with no shell. The codemod skips it but the registry script includes it (needs a `meta` export added manually or via the codemod creating a minimal `demo.config.ts`).
...
Per-demo CSS var files (`proofline-vars.css`, `proofline-route-vars.css`, `meridian-vars.css`, `phoenix-intel-vars.css`) define demo-specific tokens used by page components (chart colors, map backgrounds, animation values, etc.). These are **outside** the shell's token system scope.
...
- Namespaced per-demo (e.g., `--pl-*` for proofline, `--mr-*` for meridian)
...
5. **Keep** per-demo CSS var imports (`proofline-vars.css`, `meridian-vars.css`, etc.) — these are extension tokens, not shell tokens

**Special cases:**
- `proofline-route`: No shell, no config. Create a minimal `demo.config.ts` with `meta` fields for registry inclusion. No layout change.
- `proofline-andrews`: Treat as a standard sidebar demo (it's a fork of proofline with different nav hrefs).

**Global:**
6. Rename localStorage key: `proofline-theme` → `demo-theme` via grep+replace
7. Add runtime migration in DemoShell: on mount, if `localStorage.getItem('proofline-theme')` exists, copy value to `demo-theme` key and delete old key. This preserves existing user preferences.
...
- `styles/proofline-vars.css` is **kept** (renamed to `styles/ext/proofline.css` for clarity) — it's used by demo page components
- `styles/proofline-route-vars.css` is **kept** (renamed to `styles/ext/proofline-route.css`) — standalone page extension tokens
...
| `proofline-theme` localStorage | Runtime migration to `demo-theme` on first load |
