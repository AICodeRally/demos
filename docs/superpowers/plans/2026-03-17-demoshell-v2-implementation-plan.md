...
Port the dark/light toggle from `DemoShell.tsx`. Uses `demo-theme` localStorage key (not `routeiq-theme`). Dispatches `shell-theme-change` custom event.
...
**Global cleanup:**
- `localStorage` key: `routeiq-theme` → `demo-theme` (ThemeToggle handles this with runtime migration)
...
2. Click into at least one sidebar demo (routeiq) — sidebar renders, nav works, theme toggle works, footer has section nav row
