...
import { detectBundles, SUMMIT_SLEEP_BUNDLES } from '@/components/swic/summit-sleep/BundleBuilder';
import { ShowroomCatalog } from '@/components/swic/summit-sleep/ShowroomCatalog';
import { SaleTicket } from '@/components/swic/summit-sleep/SaleTicket';
import { CommissionPreview } from '@/components/swic/CommissionPreview';
import { CloseSaleFlow } from '@/components/swic/summit-sleep/CloseSaleFlow';
import { D365EventLog } from '@/components/swic/summit-sleep/D365EventLog';
import { Sun, Moon, Users, Wifi, RotateCcw, PauseCircle, ShoppingCart } from 'lucide-react';

// ── Page ──────────────────────────────────────────────────────

export default function SummitSleepPOSPage() {
  // ── State ─────────────────────────────────────────────────
  ...
  const result: CalculationResult | null = useMemo(
    () => (saleItems.length > 0 ? calculate(SUMMIT_SLEEP_CONFIG, saleItems, period) : null),
    [saleItems, period],
  );
  const bundles = useMemo(
    () => detectBundles(saleItems, SUMMIT_SLEEP_BUNDLES),
    [saleItems],
  );
