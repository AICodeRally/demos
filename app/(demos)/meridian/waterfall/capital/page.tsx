...
export default function CapitalAccountsPage() {
  ...
  return (
    <>
      ...
      <SectionCard title="Capital Account Summary ($M)">
        <div className="overflow-x-auto">
          <table className="w-full text-[13px]">
            <thead>
              <tr style={{ color: 'var(--mr-text-muted)' }}>
                <th className="text-left font-medium pb-3 pl-2">Limited Partner</th>
                <th className="text-right font-medium pb-3">Commitment</th>
                <th className="text-right font-medium pb-3">Called</th>
                <th className="text-right font-medium pb-3">Distributed</th>
                <th className="text-right font-medium pb-3">NAV</th>
                <th className="text-right font-medium pb-3">Unfunded</th>
            ...
            <tbody>
              {CAPITAL_ACCOUNTS.map((a, i) => (
                <tr key={a.lp} style={i % 2 === 0 ? { background: 'var(--mr-stripe)' } : undefined}>
                  ...
                  <td className="py-2.5 text-right font-mono" style={{ color: 'var(--mr-text)' }}>${a.commitment}</td>
                  <td className="py-2.5 text-right font-mono" style={{ color: 'var(--mr-text-muted)' }}>${a.called}</td>
                  <td className="py-2.5 text-right font-mono" style={{ color: '#10B981' }}>${a.distributed.toFixed(1)}</td>
                  <td className="py-2.5 text-right font-mono font-bold" style={{ color: '#D4A847' }}>${a.nav}</td>
                  <td className="py-2.5 text-right font-mono" style={{ color: 'var(--mr-text-faint)' }}>${a.unfunded}</td>
                  <td className="py-2.5 text-right font-mono font-bold pr-2" style={{ color: '#10B981' }}>{a.tvpi.toFixed(2)}x</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
              <tr style={{ borderTop: '2px solid var(--mr-border)' }}>
                ...
                <td className="py-2.5 text-right font-mono font-bold" style={{ color: '#D4A847' }}>${totalCommitment}</td>
                <td className="py-2.5 text-right font-mono font-bold" style={{ color: 'var(--mr-text)' }}>${totalCalled}</td>
                <td className="py-2.5 text-right font-mono font-bold" style={{ color: '#10B981' }}>${totalDistributed.toFixed(1)}</td>
                <td className="py-2.5 text-right font-mono font-bold" style={{ color: '#D4A847' }}>${totalNAV}</td>
                <td className="py-2.5 text-right font-mono font-bold" style={{ color: 'var(--mr-text-muted)' }}>${CAPITAL_ACCOUNTS.reduce((s, a) => s + a.unfunded, 0)}</td>
      ...
      {/* Visualization */}
      <SectionCard title="Capital Allocation Waterfall (Per LP)">
        <div className="space-y-2">
          {CAPITAL_ACCOUNTS.slice(0, 8).map((a) => {
            const calledPct = (a.called / a.commitment) * 100;
            const distPct = (a.distributed / a.commitment) * 100;
            return (
              <div key={a.lp} className="flex items-center gap-3">
                <div className="w-36 shrink-0 text-right text-[12px] font-mono truncate" style={{ color: 'var(--mr-text)' }}>
                  {a.lp}
                </div>
                ...
                <div className="w-16 text-right text-xs font-mono font-bold" style={{ color: '#D4A847' }}>
                  {a.tvpi.toFixed(2)}x
                </div>
              </div>
            );
        ...
        <div className="flex items-center gap-4 mt-3 text-xs font-mono" style={{ color: 'var(--mr-text-faint)' }}>
