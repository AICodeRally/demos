...
function AttainmentChart({ weekly, peerAvg }: { weekly: number[]; peerAvg: number[] }) {
  ...
  return (
    ...
    <svg viewBox={`0 0 ${w} ${h}`} className="w-full">
      {[0.25, 0.50, 0.75, 1.00].map(v => {
        ...
        return (
          <g key={v}>
            ...
            <text x={px - 4} y={y + 3} textAnchor="end" fontSize="8" fill="var(--pl-text-faint)" fontFamily="monospace">{(v * 100).toFixed(0)}%</text>
          </g>
        );
      })}

      <line x1={px} y1={targetY} x2={w - px} y2={targetY} stroke="#2563EB" strokeWidth="1" strokeDasharray="4 2" />
      <text x={w - px + 4} y={targetY + 3} fontSize="8" fill="#2563EB" fontFamily="monospace">100%</text>
      ...
      {weekly.map((_, i) => (
        <text key={i} x={px + (i / 12) * plotW} y={h - 2} textAnchor="middle" fontSize="7" fill="var(--pl-text-faint)" fontFamily="monospace">
          W{i + 1}
        </text>
      ))}
    </svg>
    ...
    <>
      ...
      <div className="mt-4 mb-6">
        <div className="flex items-center gap-3 mb-1">
          ...
          <span className="text-[11px] font-mono px-2 py-0.5 rounded" style={{ background: 'rgba(37,99,235,0.08)', color: '#2563EB' }}>
            {seller.routeId}
          </span>
          <span
      ...
      <div className="grid grid-cols-6 gap-3 mb-6">
        ...
        <LightKpiCard label="EMCO Gates" value={`${gates.filter(g => g.value >= g.threshold).length}/4`} accent="#2563EB" sub="Unlocked" />
      </div>
      ...
      <LightSectionCard title="13-Week Cumulative Attainment" className="mb-6">
        <div className="flex items-center gap-4 mb-2">
          ...
          <span className="text-[10px] font-mono" style={{ color: 'var(--pl-text-faint)' }}>
      ...
      {/* EMCO Gate Status */}
      <LightSectionCard title="EMCO Gate Status" className="mb-6">
        <div className="grid grid-cols-4 gap-3">
          {gates.map(g => {
            ...
            return (
              ...
              >
                <div className="text-[10px] font-mono mb-1" style={{ color: 'var(--pl-text-faint)' }}>{g.name} Gate</div>
        ...
        <div className="mt-3 text-[10px] font-mono" style={{ color: 'var(--pl-text-faint)' }}>
            ...
            {history.map(entry => (
              <div key={entry.id} className="flex gap-4">
                ...
                <div className="pb-4">
                  ...
                  <div className="flex items-center gap-2 mb-2 text-[10px] font-mono" style={{ color: 'var(--pl-text-faint)' }}>
                  ...
                  {entry.managerNotes && (
                    <p className="text-[10px] italic" style={{ color: 'var(--pl-text-faint)' }}>&mdash; {entry.managerNotes}</p>
                  )}
                </div>
              </div>
            ))}
          ...
          <div className="space-y-2">
            {coachingCards.map(card => (
              <div key={card.id} className="rounded-lg border p-3" style={{ borderColor: 'var(--pl-border)' }}>
                ...
                <p className="text-[11px]" style={{ color: 'var(--pl-text-muted)' }}>{card.suggestedAction}</p>
              </div>
            ))}
          </div>
        </LightSectionCard>
