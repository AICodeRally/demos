  ...
  return (
    ...
    <div className="flex items-center gap-3 py-1.5" style={{ paddingLeft: indent * 20 }}>
      <div className="w-40 shrink-0 text-right">
        ...
        {sub && <div className="text-xs font-mono" style={{ color: 'var(--pl-text-faint)' }}>{sub}</div>}
      </div>
          ...
          <div className="space-y-2">
            {[...ROUTES].sort((a, b) => b.attain - a.attain).slice(0, 10).map((r, i) => (
              <div key={r.id} className="flex items-center gap-3">
                <span className="text-xs font-mono w-4 text-right" style={{ color: 'var(--pl-text-faint)' }}>{i + 1}</span>
