...
function TierTrack() {
  ...
  return (
    ...
    <svg viewBox={`0 0 ${w} ${h}`} className="w-full" style={{ height: h }}>
      ...
      {tiers.map((tier, i) => {
        ...
        return (
          <g key={tier.level}>
            ...
            <text x={(x1 + x2) / 2} y={trackY - 8} textAnchor="middle" fontSize="8" fill={color} fontFamily="monospace" fontWeight="bold">
              RATE
            </text>

            {/* Tier label inside band */}
            <text x={(x1 + x2) / 2} y={trackY + trackH / 2 + 2} textAnchor="middle" fontSize="9" fontWeight="bold" fill={color} fontFamily="monospace">
      ...
      <text x={toX(marcus)} y={trackY - 34} textAnchor="middle" fontSize="8" fontWeight="bold" fill="white" fontFamily="monospace">
        MARCUS 92%
      </text>
      ...
      {EMCO_GATES.map((gate, i) => {
        ...
        return (
          <g key={gate.name}>
            ...
            <text x={cx} y={cy - r - 12} textAnchor="middle" fontSize="9" fontWeight="bold" fill={gate.color} fontFamily="monospace">
...
export default function CompPlanPage() {
  ...
  return (
    <>
      ...
      {/* EMCO Gate System */}
      <LightSectionCard title="EMCO Gate Cascade" className="mb-4">
        <EmcoGateRings />
      </LightSectionCard>
              ...
              <div className="mt-3">
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  {SPIRITS_ADDER.qualifyingBrands.map(b => (
                    <span key={b} className="text-[10px] font-mono px-2 py-0.5 rounded" style={{ background: 'rgba(16,185,129,0.1)', color: '#10B981' }}>
                      {b.replace(/-/g, ' ')}
                    </span>
                  ))}
                </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="rounded-lg border p-4" style={{ borderColor: 'var(--pl-border)' }}>
          ...
          <div className="space-y-1.5 text-[11px]" style={{ color: 'var(--pl-text-secondary)' }}>
            ...
            <div>EMCO gates: <strong className="font-mono">{plan.emcoGates.length} gates</strong></div>
