  ...
  return (
    ...
    <div className="rounded-lg border p-4" style={{ borderColor: '#E2E8F0' }}>
      <div className="flex items-center justify-between mb-2">
        <div>
          <span className="text-[9px] font-bold font-mono px-1.5 py-0.5 rounded mr-2" style={{ background: '#F1F5F9', color: '#718096' }}>
      ...
      <div className="flex items-center gap-3 mb-2">
        <div className="flex-1 text-center rounded-lg py-2" style={{ background: '#F7FAFC' }}>
          <div className="text-[9px] font-mono" style={{ color: '#A0AEC0' }}>BEFORE</div>
        ...
        <div className="flex-1 text-center rounded-lg py-2" style={{ background: 'rgba(16,185,129,0.04)' }}>
          <div className="text-[9px] font-mono" style={{ color: '#10B981' }}>AFTER</div>
          <div className="text-[18px] font-bold font-mono" style={{ color: '#1A1A2E' }}>{metric.after}</div>
        </div>
      </div>

      <p className="text-[10px]" style={{ color: '#718096' }}>{metric.description}</p>
    </div>
    </>
  );
}
...
function ImpactSummary() {
  ...
  return (
    ...
    <svg viewBox={`0 0 ${w} ${h}`} className="w-full" style={{ height: h }}>
      ...
      {impacts.map((item, i) => {
        ...
        return (
          <g key={item.label}>
            ...
            {/* Label lines */}
            {item.label.split('\n').map((line, j) => (
              <text key={j} x={cx} y={cy + 18 + j * 12} textAnchor="middle" fontSize="9" fontWeight="bold" fill="#1A1A2E" fontFamily="monospace">
                {line}
              </text>
            ))}

            {/* Sub description */}
            <text x={cx} y={cy + r + 18} textAnchor="middle" fontSize="7" fill="#A0AEC0" fontFamily="monospace">
              {item.sub}
            </text>
      ...
      <text x={w / 2} y={h - 22} textAnchor="middle" fontSize="11" fontWeight="bold" fill="#10B981" fontFamily="monospace">
  ...
  return (
    <>
      ...
      <div className="mt-6 mb-8">
        ...
        <h1 className="text-[28px] font-extrabold leading-tight" style={{ color: '#1A1A2E', fontFamily: "'Space Grotesk', sans-serif" }}>
      ...
      <LightSectionCard title="Investment vs. Return" className="mb-6">
        <div className="grid grid-cols-2 gap-6">
          {/* Investment */}
          <div>
            <div className="text-[10px] font-bold font-mono mb-3" style={{ color: '#F87171' }}>YEAR 1 INVESTMENT</div>
            <div className="space-y-2">
              {[
                ...
                <div key={row.item} className="flex items-center justify-between px-3 py-2 rounded" style={{ background: '#F7FAFC' }}>
                  <div>
                    <div className="text-[11px] font-bold" style={{ color: '#1A1A2E' }}>{row.item}</div>
                    <div className="text-[9px] font-mono" style={{ color: '#A0AEC0' }}>{row.note}</div>
                  </div>
          ...
          {/* Returns */}
          <div>
            <div className="text-[10px] font-bold font-mono mb-3" style={{ color: '#22C55E' }}>YEAR 1 RETURNS</div>
            <div className="space-y-2">
              {[
      ...
      <div className="rounded-lg p-6 mb-6" style={{ background: 'linear-gradient(135deg, rgba(16,185,129,0.06) 0%, rgba(37,99,235,0.04) 100%)', border: '1px solid rgba(16,185,129,0.2)' }}>
        ...
        <p className="text-[16px] font-bold leading-relaxed mb-4" style={{ color: '#1A1A2E', fontFamily: "'Space Grotesk', sans-serif" }}>
        ...
        <div className="flex gap-4">
          <div className="rounded-lg px-4 py-2" style={{ background: 'rgba(16,185,129,0.1)' }}>
            <span className="text-[11px] font-bold font-mono" style={{ color: '#10B981' }}>Payback: 106 days</span>
          </div>
          <div className="rounded-lg px-4 py-2" style={{ background: 'rgba(34,197,94,0.1)' }}>
            <span className="text-[11px] font-bold font-mono" style={{ color: '#22C55E' }}>ROI: 340% Year 1</span>
          </div>
          <div className="rounded-lg px-4 py-2" style={{ background: 'rgba(37,99,235,0.1)' }}>
            <span className="text-[11px] font-bold font-mono" style={{ color: '#2563EB' }}>Deploy: 8 weeks</span>
          </div>
        </div>
      </div>
