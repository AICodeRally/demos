...
function EarningsComparison() {
  ...
  return (
    ...
    <div className="grid grid-cols-2 gap-4">
      {scenarios.map(s => (
        <div key={s.label} className="rounded-xl border p-4" style={{ borderColor: `${s.color}40` }}>
          <div className="text-xs font-bold tabular-nums mb-2" style={{ color: s.color }}>{s.label}</div>

          <div className="text-[28px] font-bold tabular-nums mb-3" style={{ color: 'var(--pl-text)' }}>
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-[13px]">
              ...
              <span className="font-bold tabular-nums" style={{ color: 'var(--pl-text)' }}>${fmt(Math.round(s.data.baseEarnings))}</span>
            </div>
            <div className="flex justify-between text-[13px]">
              <span style={{ color: 'var(--pl-text-muted)' }}>Variable ({s.data.tier.label})</span>
              <span className="font-bold tabular-nums" style={{ color: 'var(--pl-text)' }}>${fmt(Math.round(s.data.variableEarnings))}</span>
            </div>
            <div className="flex justify-between text-[13px]">
              <span style={{ color: 'var(--pl-text-muted)' }}>Gate multiplier</span>
              <span className="font-bold tabular-nums" style={{ color: s.data.bbiMultiplier > 1 ? '#22C55E' : 'var(--pl-text-faint)' }}>
                {s.data.bbiMultiplier.toFixed(2)}x ({s.data.unlockedGates}/4 gates)
              </span>
            </div>
            <div className="flex justify-between text-[13px]">
              <span style={{ color: 'var(--pl-text-muted)' }}>Spirits adder</span>
              <span className="font-bold tabular-nums" style={{ color: s.data.spiritsBonus > 0 ? '#10B981' : 'var(--pl-text-faint)' }}>
            ...
            <div className="border-t pt-2 mt-2 flex justify-between text-[13px]" style={{ borderColor: 'var(--pl-border)' }}>
              ...
              <span className="font-bold tabular-nums" style={{ color: s.color }}>${fmt(Math.round(s.data.totalEstimate * 4))}</span>
            </div>
          </div>
        </div>
      ))}
    ...
    <>
      ...
      <LightSectionCard title="Commission Tier Structure" className="mb-4">
        ...
        <div className="mt-3 rounded-md px-4 py-3" style={{ background: 'rgba(16,185,129,0.04)', borderLeft: '3px solid #10B981' }}>
          <div className="text-xs font-bold tabular-nums mb-1" style={{ color: '#10B981' }}>WHY INVERSE RATES?</div>
      ...
      <LightSectionCard title="Spirits Integration Bonus" className="mb-4">
        <div className="rounded-xl p-5" style={{ background: 'rgba(16,185,129,0.04)', border: '1px solid rgba(16,185,129,0.2)' }}>
          <div className="flex items-start justify-between">
            <div>
              ...
              <div className="mt-3">
                <div className="text-xs font-bold tabular-nums" style={{ color: '#10B981' }}>QUALIFYING BRANDS</div>
                <div className="flex flex-wrap gap-1 mt-1">
                  {SPIRITS_ADDER.qualifyingBrands.map(b => (
                    <span key={b} className="text-xs px-2 py-0.5 rounded" style={{ background: 'rgba(16,185,129,0.1)', color: '#10B981' }}>
                      {b.replace(/-/g, ' ')}
                    </span>
                  ))}
                </div>
            ...
            <div className="text-right flex-shrink-0 ml-4">
              <div className="text-[32px] font-bold tabular-nums" style={{ color: '#10B981' }}>+{(SPIRITS_ADDER.rate * 100).toFixed(1)}%</div>
              <div className="text-xs" style={{ color: 'var(--pl-text-muted)' }}>on all Sazerac revenue</div>
              <div className="text-xs mt-1" style={{ color: 'var(--pl-text-faint)' }}>Min {SPIRITS_ADDER.minAccounts} spirits accounts</div>
            </div>
          </div>
        </div>
      </LightSectionCard>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="rounded-xl border p-4" style={{ borderColor: 'var(--pl-border)' }}>
          <div className="text-xs font-bold tabular-nums mb-2" style={{ color: '#2563EB' }}>PLAN MECHANICS</div>
          <div className="space-y-1.5 text-[13px]" style={{ color: 'var(--pl-text-secondary)' }}>
            <div>Pay frequency: <strong className="tabular-nums">{plan.payFrequency}</strong></div>
            <div>True-up frequency: <strong className="tabular-nums">{plan.trueUpFrequency}</strong></div>
            <div>Plan year: <strong className="tabular-nums">FY{plan.planYear}</strong></div>
            <div>gates: <strong className="tabular-nums">{plan.bbiGates.length} gates</strong></div>
            <div>Kickers: <strong className="tabular-nums">{plan.kickers.length} quarterly</strong></div>
          </div>
        </div>
        <div className="rounded-xl border p-4" style={{ borderColor: 'var(--pl-border)' }}>
          <div className="text-xs font-bold tabular-nums mb-2" style={{ color: '#22C55E' }}>OTE RANGES</div>
          <div className="space-y-1.5 text-[13px]" style={{ color: 'var(--pl-text-secondary)' }}>
            <div>Minimum OTE: <strong className="tabular-nums">${fmt(plan.ote.min)}</strong></div>
            <div>Median OTE: <strong className="tabular-nums">${fmt(plan.ote.median)}</strong></div>
            <div>Maximum OTE: <strong className="tabular-nums">${fmt(plan.ote.max)}</strong></div>
            <div>Variable %: <strong className="tabular-nums">{(plan.variableTarget * 100).toFixed(0)}% of base</strong></div>
            <div>Max multiplier: <strong className="tabular-nums">{maxMultiplier.toFixed(2)}x (combined gate)</strong></div>
          </div>
        </div>
      </div>
      ...
      <LightSectionCard title="NEW HIRE DRAW / GUARANTEE PROGRAM">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
          <div className="p-3 rounded-lg text-center" style={{ background: 'rgba(59,130,246,0.06)', border: '1px solid rgba(59,130,246,0.15)' }}>
            <div className="text-2xl font-bold tabular-nums" style={{ color: '#3B82F6' }}>${DRAW_PROGRAM.amount.toLocaleString()}</div>
            <div className="text-xs mt-1" style={{ color: 'var(--pl-text-muted)' }}>Biweekly guarantee</div>
          </div>
          <div className="p-3 rounded-lg text-center" style={{ background: 'rgba(245,158,11,0.06)', border: '1px solid rgba(245,158,11,0.15)' }}>
            <div className="text-2xl font-bold tabular-nums" style={{ color: '#F59E0B' }}>{DRAW_PROGRAM.durationMonths} months</div>
            <div className="text-xs mt-1" style={{ color: 'var(--pl-text-muted)' }}>Guarantee period</div>
          </div>
          <div className="p-3 rounded-lg text-center" style={{ background: 'rgba(34,197,94,0.06)', border: '1px solid rgba(34,197,94,0.15)' }}>
            <div className="text-2xl font-bold tabular-nums" style={{ color: '#22C55E' }}>{(DRAW_PROGRAM.recoveryPct * 100).toFixed(0)}%</div>
            <div className="text-xs mt-1" style={{ color: 'var(--pl-text-muted)' }}>Recovery rate above floor</div>
          </div>
        </div>
        ...
        <div className="mb-6">
            ...
            {DRAW_TIMELINE.map((phase) => (
              <div key={phase.phase} className="flex items-center justify-center px-3" style={{
                ...
                <div className="text-center">
                  <div className="text-xs font-bold tabular-nums" style={{ color: phase.color }}>{phase.phase}</div>
                  <div className="text-xs" style={{ color: 'var(--pl-text-faint)' }}>Mo {phase.months}</div>
                </div>
              </div>
            ))}
          </div>
          <div className="mt-2 grid grid-cols-1 sm:grid-cols-3 gap-4">
            {DRAW_TIMELINE.map((phase) => (
              <div key={phase.phase} className="text-xs" style={{ color: 'var(--pl-text-muted)' }}>
          ...
          {CURRENT_DRAW_BALANCES.map((rep) => (
            <div key={rep.rep} className="flex items-center justify-between p-3 rounded-lg"
              ...
              <div className="text-right">
                <div className="text-lg font-bold tabular-nums" style={{ color: '#F59E0B' }}>${rep.balance.toLocaleString()}</div>
                <div className="text-xs" style={{ color: 'var(--pl-text-faint)' }}>outstanding draw</div>
              </div>
            </div>
          ))}
        </div>
          ...
          {CHANNEL_COMP.map((ch) => (
            <div key={ch.channel} className="p-4 rounded-lg" style={{
              ...
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-sm font-bold tabular-nums" style={{ color: ch.color }}>
                  {ch.channel}
                </span>
              </div>
              ...
              <div className="flex justify-between text-xs mb-3" style={{ color: 'var(--pl-text-muted)' }}>
              ...
              <div className="space-y-1.5">
                {ch.variableComponents.map((vc) => (
                  <div key={vc.name} className="flex items-center gap-2">
                    ...
                    <span className="text-xs w-8 text-right" style={{ color: ch.color }}>{vc.weight}%</span>
                    <span className="text-xs flex-shrink-0" style={{ color: 'var(--pl-text-muted)', width: 200 }}>{vc.name}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="flex gap-4 mt-3 pt-3" style={{ borderTop: '1px solid var(--pl-border)' }}>
                <div className="text-center">
                  <div className="text-lg font-bold tabular-nums" style={{ color: ch.color }}>${(ch.avgOTE / 1000).toFixed(0)}K</div>
                  <div className="text-xs" style={{ color: 'var(--pl-text-faint)' }}>Avg OTE</div>
                </div>
                <div className="text-center">
                  <div className="text-lg font-bold tabular-nums" style={{ color: ch.avgAttainment >= 100 ? '#22C55E' : '#F59E0B' }}>{ch.avgAttainment}%</div>
