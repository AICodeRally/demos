...
function AttainmentChart({ weekly, peerAvg }: { weekly: number[]; peerAvg: number[] }) {
  ...
  return (
    ...
    <svg viewBox={`0 0 ${w} ${h}`} className="w-full">
      {[0.25, 0.50, 0.75, 1.00].map(v => {
        ...
        return (
          <g key={v}>
            ...
            <text x={px - 4} y={y + 3} textAnchor="end" fontSize="8" fill="#A0AEC0" fontFamily="monospace">{(v * 100).toFixed(0)}%</text>
          </g>
        );
      })}
      ...
      {weekly.map((_, i) => (
        <text key={i} x={px + (i / 12) * plotW} y={h - 2} textAnchor="middle" fontSize="7" fill="#A0AEC0" fontFamily="monospace">
          W{i + 1}
        </text>
      ))}
    </svg>
        ...
        <div className="grid grid-cols-4 gap-3">
          {gates.map(g => {
            ...
            return (
              ...
              >
                <div className="text-[10px] font-mono mb-1" style={{ color: '#A0AEC0' }}>{g.name} Gate</div>
                <div className="text-[22px] font-bold font-mono" style={{ color: unlocked ? '#22C55E' : '#F87171' }}>
                  {pct(g.value)}
                </div>
                <div className="text-[10px] font-mono" style={{ color: '#718096' }}>
            ...
            {history.map(entry => (
              <div key={entry.id} className="flex gap-4">
                ...
                <div className="pb-4">
                  ...
                  <div className="flex items-center gap-2 mb-2 text-[10px] font-mono" style={{ color: '#A0AEC0' }}>
                  ...
                  {entry.managerNotes && (
                    <p className="text-[10px] italic" style={{ color: '#A0AEC0' }}>&mdash; {entry.managerNotes}</p>
                  )}
                </div>
              </div>
            ))}
          ...
          <div className="space-y-2">
            {coachingCards.map(card => (
              <div key={card.id} className="rounded-lg border p-3" style={{ borderColor: '#E2E8F0' }}>
                ...
                <p className="text-[11px]" style={{ color: '#718096' }}>{card.suggestedAction}</p>
              </div>
            ))}
          </div>
        </LightSectionCard>
