...
function QuadrantCard({
  ...
  return (
    ...
    <Link
      href={`${prefix}${link}`}
      className="block rounded-xl border p-5 hover:shadow-md transition-shadow group"
      style={{ borderColor: 'var(--pl-border)' }}
    >
      <div className="flex items-start justify-between mb-3">
        <div>
          <div className="text-xs tracking-[2px] uppercase font-semibold mb-1" style={{ color }}>
            {subtitle}
          </div>
          <h3 className="text-[16px] font-bold" style={{ color: 'var(--pl-text)' }}>{title}</h3>
        </div>
        <div className="text-right">
          <div className="text-[22px] font-bold tabular-nums" style={{ color }}>{metric}</div>
          <div className="text-xs" style={{ color: 'var(--pl-text-faint)' }}>{metricLabel}</div>
        </div>
      </div>
      ...
      <div className="text-[13px] group-hover:underline" style={{ color }}>
        Explore &rarr;
      </div>
    </Link>
    </>
    ...
    <>
      ...
      <LightSectionCard title="Active & Upcoming Seasonal Overlays" className="mb-6">
        <div className="space-y-2">
          {SEASONAL_OVERLAYS.map(overlay => {
            ...
            return (
              <div key={overlay.eventName} className="flex items-start gap-4 px-4 py-3 rounded-xl border" style={{ borderColor: isActive ? 'rgba(37,99,235,0.3)' : 'var(--pl-border)' }}>
                <span
                ...
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs" style={{ color: 'var(--pl-text-muted)' }}>W{overlay.startWeek}–W{overlay.endWeek}</span>
                    <span className="text-xs font-bold tabular-nums" style={{ color: '#22C55E' }}>+{(overlay.impactPct * 100).toFixed(0)}%</span>
                  </div>
      ...
      >
        <div className="flex items-center gap-2 mb-3">
          <span className="text-xs font-bold tabular-nums px-1.5 py-0.5 rounded" style={{ background: 'rgba(37,99,235,0.08)', color: '#2563EB' }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            <div className="text-xs font-bold tabular-nums mb-1" style={{ color: '#22C55E' }}>DATA OWNERSHIP</div>
          ...
          <div>
            <div className="text-xs font-bold tabular-nums mb-1" style={{ color: '#2563EB' }}>HALLUCINATION PREVENTION</div>
            <p className="text-[13px]" style={{ color: 'var(--pl-text-muted)' }}>
          ...
          <div>
            <div className="text-xs font-bold tabular-nums mb-1" style={{ color: '#A855F7' }}>STACK AGNOSTIC</div>
            <p className="text-[13px]" style={{ color: 'var(--pl-text-muted)' }}>
