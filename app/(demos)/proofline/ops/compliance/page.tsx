...
function HometownCard({ summary }: { summary: HometownComplianceSummary }) {
  ...
  return (
    ...
    <div className="rounded-xl border p-4" style={{ borderColor: 'var(--pl-border)' }}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-1 text-xs mb-2">
      ...
      <div className="flex items-center justify-between text-xs" style={{ color: 'var(--pl-text-faint)' }}>
...
export default function CompliancePage() {
  ...
  return (
    <>
      ...
      {/* KPI Row */}
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mb-6 items-stretch">
      ...
      {tab === 'display' ? (
        <>
          ...
          <LightSectionCard title="Cold Vault Share — Lone Star vs Target by Hometown" className="mb-6">
            ...
            <div className="flex items-center gap-4 mt-2 text-xs" style={{ color: 'var(--pl-text-faint)' }}>
              ...
              <table className="w-full text-[12px]">
                <thead>
                  <tr style={{ color: 'var(--pl-text-muted)' }}>
                    <th className="text-left font-semibold pb-3 pl-2">Account</th>
                    <th className="text-left font-semibold pb-3">Route</th>
                    <th className="text-right font-semibold pb-3">Score</th>
                    <th className="text-left font-semibold pb-3">Status</th>
                    <th className="text-right font-semibold pb-3">Cold Vault</th>
                    <th className="text-right font-semibold pb-3">Shelf</th>
                    <th className="text-left font-semibold pb-3 pr-2">Top Issue</th>
                  </tr>
                </thead>
                <tbody>
                  {ncAccounts.map((d, i) => {
                    ...
                    return (
                      <tr key={d.accountId} style={i % 2 === 0 ? { background: 'var(--pl-stripe)' } : undefined}>
                        <td className="py-2 pl-2 font-bold tabular-nums" style={{ color: 'var(--pl-text)' }}>{d.accountId}</td>
                        <td className="py-2 tabular-nums" style={{ color: 'var(--pl-text-muted)' }}>{d.routeId}</td>
                        <td className="py-2 text-right font-bold tabular-nums" style={{ color: sc.color }}>{d.overallScore}</td>
                        <td className="py-2">
                          <span className="text-xs font-bold tabular-nums px-1.5 py-0.5 rounded" style={{ background: sc.bg, color: sc.color }}>
                            {d.status}
                          </span>
                        </td>
                        <td className="py-2 text-right tabular-nums" style={{ color: d.coldVaultShare >= d.coldVaultTarget ? '#22C55E' : '#F87171' }}>
                          {pct(d.coldVaultShare)}
                        </td>
                        <td className="py-2 text-right tabular-nums" style={{ color: d.shelfCompliance >= 0.85 ? '#22C55E' : '#F59E0B' }}>
          ...
          <LightSectionCard title="TABC License Types" className="mb-6">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {TABC_LICENSE_TYPES.map(lic => (
                <div key={lic.code} className="rounded-xl border p-3" style={{ borderColor: 'var(--pl-border)' }}>
                  ...
                  {lic.allowsSpirits && (
                    <div className="mt-1 text-xs font-bold tabular-nums" style={{ color: '#F87171' }}>SPIRITS ELIGIBLE</div>
                  )}
                </div>
              ))}
            </div>
          ...
          <LightSectionCard title="TABC Flagged Accounts" className="mb-6">
            <div className="space-y-2">
              {flaggedTABC.map(a => {
                ...
                return (
                  <div key={a.accountId} className="flex items-center gap-4 px-4 py-3 rounded-xl border" style={{ borderColor: sc.color + '30' }}>
                    <span className="text-[12px] font-bold tabular-nums" style={{ color: 'var(--pl-text)' }}>{a.accountId}</span>
                    <span
                    ...
                    <span className="text-[13px]" style={{ color: 'var(--pl-text-muted)' }}>{a.licenseCode} · #{a.licenseNumber}</span>
                    <span className="text-[13px]" style={{ color: 'var(--pl-text-muted)' }}>Exp: {a.expirationDate}</span>
          ...
          <LightSectionCard title="TABC Issues by Hometown" className="mb-6">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {HOMETOWN_COMPLIANCE.map(h => {
                const ht = HOMETOWNS.find(x => x.id === h.hometownId);
                return (
                  <div key={h.hometownId} className="rounded-xl border p-3 flex items-center justify-between" style={{ borderColor: 'var(--pl-border)' }}>
      ...
      <LightSectionCard title="ACCOUNTS RECEIVABLE & CREDIT HOLDS">
          ...
          <div className="flex rounded-xl overflow-hidden mb-3" style={{ height: 32, border: '1px solid var(--pl-border)' }}>
            {AR_AGING.map((bucket) => (
              <div key={bucket.bucket} className="flex items-center justify-center" style={{
                ...
                {bucket.pct > 10 && (
                  <span className="text-xs font-bold tabular-nums" style={{ color: bucket.color }}>{bucket.pct}%</span>
                )}
              </div>
            ))}
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {AR_AGING.map((bucket) => (
              <div key={bucket.bucket} className="p-3 rounded-lg text-center" style={{
                ...
                <div className="text-lg font-bold tabular-nums" style={{ color: bucket.color }}>
                  ${(bucket.amount / 1000).toFixed(0)}K
                </div>
                <div className="text-xs mt-1" style={{ color: 'var(--pl-text-muted)' }}>{bucket.bucket}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="text-xs font-bold tabular-nums uppercase tracking-widest mb-3" style={{ color: '#F87171' }}>
            ...
            <div key={acct.account} className="flex items-center justify-between p-3 rounded-lg"
              ...
              <div className="flex items-center gap-3">
                <div className="text-right">
                  <div className="text-sm font-bold tabular-nums" style={{ color: '#F87171' }}>${acct.balance.toLocaleString()}</div>
                  <div className="text-xs" style={{ color: 'var(--pl-text-faint)' }}>outstanding</div>
                </div>
                <span className="px-2 py-1 rounded text-xs font-bold tabular-nums"
                  style={{
