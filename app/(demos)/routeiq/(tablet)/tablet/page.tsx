...
import { ROYAL_CONFIG, ROYAL_CATALOG, ROYAL_PERIODS } from '@/data/routeiq/royal-config';
...
export default function RouteiqTabletPage() {
  ...
  const allSaleItems = useMemo(() => {
    ...
    for (const stop of ROUTE_STOPS) {
      for (const m of stop.manifest) {
        const catalogItem = ROYAL_CATALOG.find((c) => c.id === m.productId);
    ...
    for (const stop of ROUTE_STOPS) {
      ...
      for (const sugId of acceptedIds) {
        ...
        const catalogItem = ROYAL_CATALOG.find((c) => c.id === suggestion.productId);
        if (!catalogItem) continue;
  ...
  const period = ROYAL_PERIODS[ROUTE_META.repId];

  const result = useMemo(() => {
    if (allSaleItems.length === 0) return null;
    return calculate(ROYAL_CONFIG, allSaleItems, period);
  }, [allSaleItems, period]);

  const baselineResult = useMemo(() => {
    if (baselineItems.length === 0) return null;
    return calculate(ROYAL_CONFIG, baselineItems, period);
