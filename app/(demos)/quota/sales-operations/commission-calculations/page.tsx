...
const tierColors: Record<string, string> = {
  'Base': 'border-red-400/30 bg-red-400/10 text-red-600',
  'Standard': 'border-amber-400/30 bg-amber-400/10 text-amber-400',
  'Accelerator': 'border-cyan-400/30 bg-cyan-400/10 text-cyan-600',
  'Super Accelerator': 'border-emerald-400/30 bg-emerald-400/10 text-emerald-600',
};
    ...
    <>
            ...
            <div key={t.tier} className="relative">
              ...
              {i < COMMISSION_TIERS.length - 1 && (
                <div className="hidden md:flex absolute -right-3 top-1/2 -translate-y-1/2 z-10">
                  <ChevronRight className="h-5 w-5" style={{ color: 'var(--prizym-text-muted)' }} />
                </div>
              )}
            </div>
          ))}
      ...
      <div className="grid gap-6 lg:grid-cols-3 mb-6">
        ...
        <div className="lg:col-span-2 rounded-xl p-5" style={{ background: 'var(--prizym-card-bg)', border: '1px solid var(--prizym-border-default)', boxShadow: 'var(--prizym-shadow-card)' }}>
          <h2 className="text-sm font-semibold mb-4" style={{ color: 'var(--prizym-text-primary)' }}>Commission Earned by Rep</h2>
          <div className="h-72">
        ...
        <div className="rounded-xl p-5" style={{ background: 'var(--prizym-card-bg)', border: '1px solid var(--prizym-border-default)', boxShadow: 'var(--prizym-shadow-card)' }}>
          <h2 className="text-sm font-semibold mb-4" style={{ color: 'var(--prizym-text-primary)' }}>Tier Distribution</h2>
          <div className="h-48">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
            ...
            {tierCounts.map(t => (
              <div key={t.name} className="flex items-center justify-between text-xs">
                <div className="flex items-center gap-2">
                  ...
                  <span style={{ color: 'var(--prizym-text-muted)' }}>{t.name}</span>
                </div>
      ...
      <div className="rounded-xl p-5" style={{ background: 'var(--prizym-card-bg)', border: '1px solid var(--prizym-border-default)', boxShadow: 'var(--prizym-shadow-card)' }}>
        <h2 className="text-sm font-semibold mb-4" style={{ color: 'var(--prizym-text-primary)' }}>Commission Detail</h2>
